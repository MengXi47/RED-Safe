...
FallProbInfer::FallProbInfer(const std::string& ts_model_path) {
	try {
		...
		throw std::runtime_error(std::string("Load TorchScript failed: ") + e.what());
	}
	device_ = selectDevice();
	if (device_.is_cuda()) module_.to(device_);
}

torch::Tensor FallProbInfer::toTensor(const std::vector<std::array<float, 9>>& batch) {
	const int64_t n = static_cast<int64_t>(batch.size());
	torch::Tensor t = torch::empty({n, 9}, torch::TensorOptions().dtype(torch::kFloat32));
	// 逐行拷貝，避免 from_blob 的生命週期/對齊問題
	for (int64_t i = 0; i < n; ++i) {
		auto row = torch::from_blob((void*)batch[i].data(), {9}, torch::kFloat32).clone();
		t.index_put_({i}, row);
	}
	return t.to(device_);
}
...
std::vector<float> FallProbInfer::inferBatch(const std::vector<std::array<float, 9>>& batch) {
