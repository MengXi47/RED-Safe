...
void UdpSocket::SendTo(std::string_view data, const sockaddr_in& dest) const {
  auto sent = ::sendto(handle_, data.data(), data.size(), 0,
...
std::optional<std::pair<std::string, std::string>> UdpSocket::ReceiveFrom() const {
  sockaddr_in sender{};
  socklen_t sender_len = sizeof(sender);
  char buffer[kBufferSize];

  auto received = ::recvfrom(handle_, buffer, sizeof(buffer), 0,
  ...
  return std::make_pair(std::string{ip_buffer},
