...
constexpr std::string_view kScopeTags[] = {
    "d:Scopes", "Scopes", "wsd:Scopes", "wsdd:Scopes"};
...
void OnvifResponseParser::Parse(
    std::string_view xml, DeviceInfo& device) const {
...
std::optional<std::string_view> OnvifResponseParser::ExtractTag(
  ...
  while ((search_pos = xml.find('<', search_pos)) != std::string_view::npos) {
    ...
    if (!(xml[after_tag] == '>' ||
          std::isspace(static_cast<unsigned char>(xml[after_tag])))) {
      ++search_pos;
      continue;
    }
...
void OnvifResponseParser::ParseScopes(
    std::string_view scopes, DeviceInfo& device) const {
  if (device.mac.empty()) {
