...
std::string ConfigLoader::GetEnvOrDefault(const char* name, std::string default_value) {
...
std::optional<int> ConfigLoader::GetEnvInt(const char* name) {
    if (const char* value = std::getenv(name)) {
        try {
            ...
            LogWarnFormat("環境變數 {} 無法轉為整數: {} - {}", name, value, ex.what());
        } catch (...) {
...
EdgeConfig ConfigLoader::Load() const {
    ...
    cfg.server_base_url = GetEnvOrDefault("RED_SAFE_SERVER_URL", "https://api.redsafe-tw.com");
    cfg.mqtt_broker = GetEnvOrDefault("RED_SAFE_MQTT_BROKER", "mqtt.redsafe-tw.com/mqtt");
    cfg.mqtt_port = static_cast<std::uint16_t>(GetEnvInt("RED_SAFE_MQTT_PORT").value_or(443));
    ...
    if (auto hb_ms = GetEnvInt("RED_SAFE_HEARTBEAT_MS")) {
        cfg.heartbeat_interval = std::chrono::milliseconds(std::max(kMinHeartbeatMs, *hb_ms));
    }
    if (auto scan_ms = GetEnvInt("RED_SAFE_IPCSCAN_TIMEOUT_MS")) {
        cfg.ipcscan_timeout = std::chrono::milliseconds(std::max(kMinScanTimeoutMs, *scan_ms));
    }
    return cfg;
}
