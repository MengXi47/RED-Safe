...
util::Result IOSAPPService::start(const std::string &ios_device_id,
  ...
  if (!is_vaild_apns(apns_token))
    return util::Result{util::status_code::BadRequest,
                        util::error_code::Invalid_apnstoken_format, json{}};

  if (IOSDeviceRegistrar::start(ios_device_id, user_id, apns_token,
                                device_name) == 1)
  ...
  if (auto ios_device_id_ = IOSDeviceIDFinder::start(apns_token);
      ios_device_id_.empty())
    return util::Result{util::status_code::InternalServerError,
                        util::error_code::Internal_server_error, json{}};
  else
    return util::Result{util::status_code::Success, util::error_code::Success,
