...
Controller::Controller(http::request<http::string_body> req) : req_(std::move(req)) {}

response Controller::handle_request() const {
  static const folly::F14FastMap<
      folly::StringPiece, std::function<util::Result(const http::request<http::string_body>&)>>
      POST_map = {
          {"/edge/signup",
           [](const http::request<http::string_body>& req) {
             try {
               ...
               return service::edge::Register::start(body.value("version", std::string{}),
          ...
          {"/user/signup",
           [](const http::request<http::string_body>& req) {
             try {
               ...
               return service::User::signup::run(body.value("email", std::string{}),
          ...
          {"/user/signin",
           [](const http::request<http::string_body>& req) {
             try {
               ...
               return service::User::signin::run(body.value("email", std::string{}),
                                                 body.value("password", std::string{}));
             } catch ([[maybe_unused]] nlohmann::json::parse_error& e) {
               return util::Result{util::status_code::BadRequest, util::error_code::Invalid_JSON,
                                   json{}};
           ...
           [](const http::request<http::string_body>& req) {
             try {
               ...
               return service::IOSAPP::IOSAPPService::start(
                   body.value("ios_device_id", std::string{}), body.value("user_id", std::string{}),
          ...
          {"/ios/bind",
           [](const http::request<http::string_body>& req) {
             try {
               ...
               return service::User::Binding::bind(body.value("serial_number", std::string{}),
                                                   access_token);
             } catch ([[maybe_unused]] nlohmann::json::parse_error& e) {
               return util::Result{util::status_code::BadRequest, util::error_code::Invalid_JSON,
                                   json{}};
          ...
          {"/ios/unbind",
           [](const http::request<http::string_body>& req) {
             try {
               ...
               return service::User::Binding::unbind(body.value("serial_number", std::string{}),
                                                     access_token);
             } catch ([[maybe_unused]] nlohmann::json::parse_error& e) {
               return util::Result{util::status_code::BadRequest, util::error_code::Invalid_JSON,
                                   json{}};
...
response Controller::make_response(int status_code, const json& j, const std::string_view cookie) {
  response res{static_cast<http::status>(status_code), 11};
  res.keep_alive(true);
  res.set(http::field::server, "RED-Safe");
  res.set(http::field::content_type, "application/json");
...
std::string Controller::get_access_token(const http::request<http::string_body>& req) {
  ...
  if (req.base().count("Authorization")) {
    ...
    if (constexpr std::string_view prefix = "Bearer ";
        authHeader.size() > prefix.size() && authHeader.substr(0, prefix.size()) == prefix) {
