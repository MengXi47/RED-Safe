...
void Session::start() {
  boost::asio::co_spawn(
      socket_.get_executor(), [self = shared_from_this()]() { return self->run(); },
      boost::asio::detached);
}
...
boost::asio::awaitable<void> Session::run() {
  try {
    for (;;) {
      ...
      auto raw_req = req_;
#ifndef NDEBUG
      util::cout() << util::current_timestamp() << req_.base()["X-Real-IP"] << " " << req_.method()
                   << " " << req_.target() << " " << req_.body() << '\n';
      util::log(util::LogFile::access, util::Level::INFO)
          << req_.base()["X-Real-IP"] << " " << req_.method() << " " << req_.target() << " "
          << req_.body();
#endif
    ...
    util::cout() << util::current_timestamp()
                 << "nginx disconnection: " << socket_.remote_endpoint().address().to_string()
