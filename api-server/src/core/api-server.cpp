...
class Server::Impl {
 public:
  explicit Impl(const uint16_t port) : acceptor_{io_, {tcp::v4(), port}} { acceptor_.listen(1024); }
 ...
 private:
  void do_accept() {
    acceptor_.async_accept([this](auto ec, tcp::socket socket) {
      if (ec) {
        util::log(util::LogFile::server, util::Level::ERROR) << "Accept failed: " << ec.message();
      ...
      util::cout() << util::current_timestamp() << "nginx connection: " << addr << ':' << port
                   << '\n';
    });
  }
  ...
  void threadscreater() {
    ...
    for (unsigned int i = 0; i < numThreads; ++i) post(*pool_, [ctx = &io_] { ctx->run(); });
#endif
#endif
  }
...
void Server::start() const { impl_->start(); }

void Server::stop() const { impl_->stop(); }
}  // namespace redsafe::apiserver
