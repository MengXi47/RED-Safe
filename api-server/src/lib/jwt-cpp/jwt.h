 ...
 *
 * A namespace to contain everything related to handling JSON Web Tokens, JWT
 * for short, as a part of [RFC7519](https://tools.ietf.org/html/rfc7519), or
 * alternatively for JWS (JSON Web Signature) from
...
template <>
struct is_error_code_enum<jwt::error::signature_verification_error>
    : true_type {};
template <>
 ...
 * For example, when dealing with JWKS (JSON Web Key
 * Set)[https://tools.ietf.org/html/rfc7517] you maybe need to extract the
 ...
 *
 * Starting from OpenSSL 1.1.0, EVP_PKEY has internal reference counting. This
 * handle class allows jwt-cpp to leverage that and thus safe an allocation for
 * the control block in std::shared_ptr. The handle uses shared_ptr as a
 * fallback on older versions. The behaviour should be identical between both.
 */
class evp_pkey_handle {
 public:
   ...
   */
  explicit evp_pkey_handle(EVP_PKEY* key) {
...
std::string write_bio_to_string(
    std::unique_ptr<BIO, decltype(&BIO_free_all)>& bio_out,
    std::error_code& ec) {
  char* ptr = nullptr;
 ...
 *
 * \tparam error_category	jwt::error enum category to match with the keys
 * being used
 * \param certstr			String containing the certificate
 * encoded as pem
 * \param pw				Password used to decrypt certificate
 * (leave empty if not encrypted)
 * \param ec				error_code for error_detection (gets
 * cleared if no error occurred)
 */
template <typename error_category = error::rsa_error>
std::string extract_pubkey_from_cert(
    const std::string& certstr, const std::string& pw, std::error_code& ec) {
  ec.clear();
 ...
 * \param pw				Password used to decrypt certificate
 * (leave empty if not encrypted)
 * \throw					templated error_category's type
 * exception if an error occurred
 */
template <typename error_category = error::rsa_error>
std::string extract_pubkey_from_cert(
    const std::string& certstr, const std::string& pw = "") {
  std::error_code ec;
 ...
 * \param ec			error_code for error_detection (gets cleared if
 * no error occurs)
 */
inline std::string convert_der_to_pem(
    const std::string& cert_der_str, std::error_code& ec) {
  ec.clear();
 ...
 *
 * This is useful when using with JWKs as x5c claim is encoded as base64 DER.
 * More info [here](https://tools.ietf.org/html/rfc7517#section-4.7).
 *
 ...
 * (gets cleared if no error occurs)
 */
...
template <typename Decode>
std::string convert_base64_der_to_pem(
    const std::string& cert_base64_der_str, Decode decode) {
  std::error_code ec;
  auto res =
      convert_base64_der_to_pem(cert_base64_der_str, std::move(decode), ec);
  error::throw_if_error(ec);
  return res;
}
...
inline std::string convert_base64_der_to_pem(
    const std::string& cert_base64_der_str, std::error_code& ec) {
 ...
 * \param password	Password used to decrypt certificate (leave empty if not
 * encrypted)
 * \param ec		error_code for error_detection (gets cleared if no error
 * occurs)
 */
template <typename error_category = error::rsa_error>
evp_pkey_handle load_public_key_from_string(
    const std::string& key, const std::string& password, std::error_code& ec) {
  ec.clear();
  ...
  if (key.substr(0, 27) == "-----BEGIN CERTIFICATE-----") {
    auto epkey =
        helper::extract_pubkey_from_cert<error_category>(key, password, ec);
    if (ec)
      return {};
  ...
  evp_pkey_handle pkey(PEM_read_bio_PUBKEY(
      ...
      nullptr,
      nullptr,
      (void*)password.data())); // NOLINT(google-readability-casting) requires
                                // `const_cast`
  if (!pkey)
 ...
 * \tparam error_category	jwt::error enum category to match with the keys
 * being used
 * \param key				String containing the certificate
 * encoded as pem
 * \param password			Password used to decrypt certificate
 * (leave empty if not encrypted)
 * \throw					Templated error_category's type
 * exception if an error occurred
 */
template <typename error_category = error::rsa_error>
inline evp_pkey_handle load_public_key_from_string(
 ...
 * \param password			Password used to decrypt key (leave
 * empty if not encrypted)
 * \param ec				error_code for error_detection (gets
 * cleared if no error occurs)
 */
template <typename error_category = error::rsa_error>
inline evp_pkey_handle load_private_key_from_string(
    const std::string& key, const std::string& password, std::error_code& ec) {
  ec.clear();
  ...
  evp_pkey_handle pkey(PEM_read_bio_PrivateKey(
      private_key_bio.get(),
      nullptr,
      nullptr,
 ...
 * The string should contain a pem encoded certificate or public key
 *
 * \deprecated Use the templated version helper::load_private_key_from_string
 * with error::ecdsa_error
 *
 * \param key		String containing the certificate encoded as pem
 * \param password	Password used to decrypt certificate (leave empty if not
 * encrypted)
 ...
 */
inline std::unique_ptr<BIGNUM, decltype(&BN_free)> raw2bn(
 ...
 * \param password	Password used to decrypt certificate or key (leave empty
 * if not encrypted)
 ...
 *
 * \param key		String containing a private key as pem
 * \param password	Password used to decrypt key (leave empty if not
 * encrypted)
 * \param ec		error_code for error_detection (gets cleared if no error
 * occurs)
 */
inline evp_pkey_handle load_private_ec_key_from_string(
    const std::string& key, const std::string& password, std::error_code& ec) {
 ...
 */
template <typename Decode>
std::string create_public_key_from_rsa_components(
  ...
  std::unique_ptr<OSSL_PARAM_BLD, decltype(&OSSL_PARAM_BLD_free)> param_bld(
  ...
  // https://www.openssl.org/docs/manmaster/man3/EVP_PKEY_fromdata_init.html#RETURN-VALUES
  EVP_PKEY* pkey = NULL;
  if (EVP_PKEY_fromdata_init(ctx.get()) <= 0 ||
      EVP_PKEY_fromdata(ctx.get(), &pkey, EVP_PKEY_KEYPAIR, params.get()) <=
          0) {
...
template <typename Decode>
std::string create_public_key_from_rsa_components(
    const std::string& modulus, const std::string& exponent, Decode decode) {
  std::error_code ec;
  auto res =
      create_public_key_from_rsa_components(modulus, exponent, decode, ec);
  error::throw_if_error(ec);
  return res;
}
 ...
 * \return public key in PEM format
 */
inline std::string create_public_key_from_rsa_components(
    const std::string& modulus,
    const std::string& exponent,
    std::error_code& ec) {
  auto decode = [](const std::string& token) {
    return base::decode<alphabet::base64url>(
        base::pad<alphabet::base64url>(token));
  };
  return create_public_key_from_rsa_components(
      modulus, exponent, std::move(decode), ec);
}
/**
 * Create public key from modulus and exponent. This is defined in
 * [RFC 7518 Section 6.3](https://www.rfc-editor.org/rfc/rfc7518#section-6.3)
 ...
 */
inline std::string create_public_key_from_rsa_components(
    const std::string& modulus, const std::string& exponent) {
  std::error_code ec;
...
inline evp_pkey_handle load_private_ec_key_from_string(
  ...
  std::error_code ec;
  auto res =
      load_private_key_from_string<error::ecdsa_error>(key, password, ec);
  error::throw_if_error(ec);
  return res;
}
 ...
 * Using the required "crv" (Curve), "x" (X Coordinate) and "y" (Y Coordinate)
 * Parameters.
 *
 * \tparam Decode is callable, taking a string_type and returns a string_type.
 * It should ensure the padding of the input and then base64url decode and
 ...
 */
template <typename Decode>
std::string create_public_key_from_ec_components(
  ...
  if (OSSL_PARAM_BLD_push_utf8_string(
          param_bld.get(), "group", group.data(), group.size()) != 1 ||
      OSSL_PARAM_BLD_push_octet_string(
          param_bld.get(), "pub", pub.data(), pub.size()) != 1) {
  ...
  std::unique_ptr<EC_GROUP, decltype(&EC_GROUP_free)> ecgroup(
  ...
  std::unique_ptr<EC_POINT, decltype(&EC_POINT_free)> ecpoint(
      EC_POINT_new(ecgroup.get()), EC_POINT_free);
  if (!ecpoint ||
      EC_POINT_set_affine_coordinates_GFp(
          ecgroup.get(), ecpoint.get(), qx.get(), qy.get(), nullptr) != 1) {
    ec = error::ecdsa_error::set_ecdsa_failed;
    return {};
  }

  std::unique_ptr<EC_KEY, decltype(&EC_KEY_free)> ecdsa(
...
inline std::string create_public_key_from_ec_components(
  ...
  return create_public_key_from_ec_components(
      curve, x, y, std::move(decode), ec);
}
/**
 * Create public key from curve name and coordinates. This is defined in
 * [RFC 7518 Section 6.2](https://www.rfc-editor.org/rfc/rfc7518#section-6.2)
 ...
 */
inline std::string create_public_key_from_ec_components(
    const std::string& curve, const std::string& x, const std::string& y) {
  std::error_code ec;
 ...
 *
 * JWT (JSON Web Tokens) signatures are typically used as the payload for a JWS
 * (JSON Web Signature) or JWE (JSON Web Encryption). Both of these use various
 * cryptographic as specified by [RFC7518](https://tools.ietf.org/html/rfc7518)
 * and are exposed through the a [JOSE
 * Header](https://tools.ietf.org/html/rfc7515#section-4) which points to one of
 * the JWA [JSON Web
 ...
 * See [RFC 7518
 * Section 3.6](https://datatracker.ietf.org/doc/html/rfc7518#section-3.6) for
 * more information.
 */
struct none {
  /**
...
struct rsa {
   ...
   * empty, signing will always fail.
   ...
   * Construct new rsa algorithm
   *
   * \param key_pair openssl EVP_PKEY structure containing RSA key pair. The
   * private part is optional.
   * \param md Pointer to hash function
   * \param name Name of the algorithm
   */
  ...
  std::string sign(const std::string& data, std::error_code& ec) const {
    ...
    if (EVP_SignFinal(
            ctx.get(), (unsigned char*)res.data(), &len, pkey.get()) == 0) {
...
struct ecdsa {
  ...
  ecdsa(
    ...
    if (!private_key.empty()) {
      pkey = helper::load_private_ec_key_from_string(
          private_key, private_key_password);
      check_private_key(pkey.get());
    } else if (!public_key.empty()) {
      pkey = helper::load_public_ec_key_from_string(
          public_key, public_key_password);
   ...
   * Construct new ecdsa algorithm
   *
   * \param key_pair openssl EVP_PKEY structure containing ECDSA key pair. The
   * private part is optional.
   * \param md Pointer to hash function
   * \param name Name of the algorithm
   * \param siglen The bit length of the signature
   */
  ecdsa(
      helper::evp_pkey_handle key_pair,
      const EVP_MD* (*md)(),
      std::string name,
      size_t siglen)
      : pkey(std::move(key_pair)),
        md(md),
...
#endif
    auto res = EVP_DigestVerifyFinal(
        ctx.get(),
        der_sig_data,
 ...
 private:
  ...
  std::string der_to_p1363_signature(
      const std::string& der_signature, std::error_code& ec) const {
  ...
  std::string p1363_to_der_signature(
      const std::string& signature, std::error_code& ec) const {
    ec.clear();
 ...
 * v1.1.1](https://www.openssl.org/news/openssl-1.1.1-notes.html), so these
 * algorithms are only available when building against this version or higher.
 */
struct eddsa {
  /**
   * Construct new eddsa algorithm
   * \param public_key EdDSA public key in PEM format
   * \param private_key EdDSA private key or empty string if not available. If
   * empty, signing will always fail.
   * \param public_key_password Password to decrypt public key pem.
...
// LibreSSL is the special kid in the block, as it does not support
// EVP_DigestSign. OpenSSL on the otherhand does not support using
// EVP_DigestSignUpdate for eddsa, which is why we end up with this mess.
    ...
    if (EVP_DigestSignFinal(
            ctx.get(), reinterpret_cast<unsigned char*>(&res[0]), &len) != 1) {
      ec = error::signature_generation_error::signfinal_failed;
      return {};
    }
#else
    if (EVP_DigestSign(
            ctx.get(),
            reinterpret_cast<unsigned char*>(&res[0]),
            &len,
            reinterpret_cast<const unsigned char*>(data.data()),
            data.size()) != 1) {
      ec = error::signature_generation_error::signfinal_failed;
  ...
  void verify(
    ...
    if (!EVP_DigestVerifyInit(
            ctx.get(), nullptr, nullptr, nullptr, pkey.get())) {
      ec = error::signature_verification_error::verifyinit_failed;
      return;
    }
// LibreSSL is the special kid in the block, as it does not support
// EVP_DigestVerify. OpenSSL on the otherhand does not support using
// EVP_DigestVerifyUpdate for eddsa, which is why we end up with this mess.
#if defined(LIBRESSL_VERSION_NUMBER) || defined(LIBWOLFSSL_VERSION_HEX)
...
struct pss {
  ...
  std::string sign(const std::string& data, std::error_code& ec) const {
    ...
    if (EVP_DigestSignInit(md_ctx.get(), &ctx, md(), nullptr, pkey.get()) !=
        1) {
      ec = error::signature_generation_error::signinit_failed;
      return {};
    }
...
// wolfSSL does not require EVP_PKEY_CTX_set_rsa_pss_saltlen. The default
// behavior sets the salt length to the hash length. Unlike OpenSSL which
// exposes this functionality.
  ...
  void verify(
    ...
    EVP_PKEY_CTX* ctx = nullptr;
    if (EVP_DigestVerifyInit(md_ctx.get(), &ctx, md(), nullptr, pkey.get()) !=
        1) {
      ec = error::signature_verification_error::verifyinit_failed;
      return;
    }
...
// exposes this functionality.
    ...
    if (EVP_DigestVerifyFinal(
            md_ctx.get(), (unsigned char*)signature.data(), signature.size()) <=
        0) {
      ec = error::signature_verification_error::verifyfinal_failed;
      return;
    }
 ...
 * RS256 algorithm.
 *
 * This data structure is used to describe the RSA256 and can be used to verify
 * JWTs
 */
...
struct ed25519 : public eddsa {
   ...
   * \param private_key Ed25519 private key or empty string if not available. If
   * empty, signing will always fail.
   * \param public_key_password Password to decrypt public key pem.
   * \param private_key_password Password
   * to decrypt private key pem.
...
struct ed448 : public eddsa {
   ...
   * \param private_key Ed448 private key or empty string if not available. If
   * empty, signing will always fail.
   * \param public_key_password Password to decrypt public key pem.
   * \param private_key_password Password
   * to decrypt private key pem.
 ...
 *
 * This enum is to abstract the third party underlying types and allows the
 * library to identify the different structures and reason about them without
 * needing a "concept" to capture that defintion to compare against a concrete
 * type.
 */
...
struct is_function_signature_detected {
  ...
  static constexpr auto value = is_detected<Op, traits_type>::value &&
...
struct supports_get_type {
  ...
  static constexpr auto value = is_function_signature_detected<
      traits_type,
      get_type_t,
      json::type(const value_type&)>::value;
...
#define JWT_CPP_SUPPORTS_AS(TYPE)                                      \
  template <                                                           \
      typename traits_type,                                            \
      typename value_type,                                             \
      typename JWT_CPP_JSON_TYPE_TYPE(TYPE)>                           \
  struct supports_as_##TYPE {                                          \
    ...
    static constexpr auto value = is_function_signature_detected<      \
        traits_type,                                                   \
        JWT_CPP_AS_TYPE_T(TYPE),                                       \
        JWT_CPP_JSON_TYPE_TYPE(TYPE)(const value_type&)>::value;       \
                                                                       \
    static_assert(                                                     \
        value,                                                         \
        "traits implementation must provide `" #TYPE "_type as_" #TYPE \
        "(const value_type&)`");                                       \
  }
...
struct is_valid_json_value {
  static constexpr auto value =
      ...
      std::is_constructible<value_type, const value_type&>::
          value && // a more generic is_copy_constructible
...
struct is_subcription_operator_signature<
    object_type,
    string_type,
    void_t<decltype(std::declval<object_type>().operator[](
        std::declval<string_type>()))>> : std::true_type {
...
using is_at_const_signature = typename std::is_same<
    decltype(std::declval<const object_type>().at(
        std::declval<const string_type>())),
    const value_type&>;
...
struct is_valid_json_object {
  ...
  static constexpr auto value =
      ...
      (std::is_same<typename object_type::key_type, string_type>::value ||
       std::is_constructible<typename object_type::key_type, string_type>::
           value) &&
      is_detected<iterator_t, object_type>::value &&
      is_detected<const_iterator_t, object_type>::value &&
      is_iterable<object_type>::value &&
...
using is_substr_start_end_index_signature = typename std::is_same<
...
using is_substr_start_index_signature = typename std::is_same<
    decltype(std::declval<string_type>().substr(std::declval<integer_type>())),
    string_type>;
...
struct is_valid_json_string {
  ...
  static constexpr auto value =
      std::is_constructible<value_type, string_type>::value && substr &&
      operator_plus;
};
...
class basic_claim {
   ...
   */
  static_assert(
      std::is_same<typename json_traits::string_type, std::string>::value ||
          std::is_convertible<typename json_traits::string_type, std::string>::
              value ||
 ...
 public:
  ...
  date as_date() const {
    ...
    if (get_type() == json::type::number)
      return system_clock::from_time_t(
          static_cast<std::time_t>(std::round(as_number())));
   ...
   */
  typename json_traits::integer_type as_integer() const {
   ...
   */
  typename json_traits::boolean_type as_boolean() const {
...
struct map_of_claims {
   ...
   */
  static typename json_traits::object_type parse_claims(
...
class payload {
 ...
 public:
   ...
   * \throw std::bad_cast Claim was present but not a string (Should not happen
   * in a valid token)
   */
   ...
   * \throw std::runtime_error If claim was not present
   * \throw std::bad_cast Claim was present but not a set (Should not happen in
   * a valid token)
   */
   ...
   * \throw std::runtime_error If claim was not present
   * \throw std::bad_cast Claim was present but not a date (Should not happen in
   * a valid token)
   */
   ...
   * \return true if claim was present, false otherwise
   */
  bool has_payload_claim(
      const typename json_traits::string_type& name) const noexcept {
   ...
   * \throw std::runtime_error If claim was not present
   */
  basic_claim_t get_payload_claim(
      const typename json_traits::string_type& name) const {
...
class header {
 ...
 public:
   ...
   * \throw std::runtime_error If claim was not present
   */
  basic_claim_t get_header_claim(
      const typename json_traits::string_type& name) const {
 ...
 public:
  ...
  JWT_CLAIM_EXPLICIT decoded_jwt(const typename json_traits::string_type& token)
      : decoded_jwt(token, [](const typename json_traits::string_type& str) {
          return base::decode<alphabet::base64url>(
              base::pad<alphabet::base64url>(str));
        }) {}
#endif
  /**
   * \brief Parses a given token
  ...
  decoded_jwt(const typename json_traits::string_type& token, Decode decode)
    ...
    this->header_claims =
        details::map_of_claims<json_traits>::parse_claims(header);
    this->payload_claims =
        details::map_of_claims<json_traits>::parse_claims(payload);
  }
   ...
   */
  const typename json_traits::string_type& get_signature_base64()
      const noexcept {
   ...
   */
  typename json_traits::object_type get_payload_json() const {
   ...
   * \return map of claims
   */
  typename json_traits::object_type get_header_json() const {
...
class builder {
 ...
 public:
  ...
  builder& set_header_claim(
      const typename json_traits::string_type& id,
   ...
   */
  builder& set_header_claim(
      const typename json_traits::string_type& id, basic_claim<json_traits> c) {
   ...
   * \tparam Algo Callable method which takes a string_type and return the
   * signed input as a string_type
   * \tparam Encode Callable method which takes a string_type and base64url safe
   * encodes it, MUST return the result with no padding; trim the result.
   * \param algo Instance of an algorithm to sign the token with
   * \param encode Callable to transform the serialized json to base64 with no
   * padding
   * \return Final token as a string
   *
   * \note If the 'alg' header in not set in the token it will be set to
   * `algo.name()`
   */
  template <typename Algo, typename Encode>
  typename json_traits::string_type sign(
      const Algo& algo, Encode encode) const {
    std::error_code ec;
  ...
  template <typename Algo, typename Encode>
  typename json_traits::string_type sign(
      const Algo& algo, Encode encode, std::error_code& ec) const {
    ...
    const auto header = encode(
        json_traits::serialize(typename json_traits::value_type(obj_header)));
    const auto payload = encode(
        json_traits::serialize(
  ...
  template <typename Algo>
  typename json_traits::string_type sign(
      const Algo& algo, std::error_code& ec) const {
    return sign(
        algo,
...
struct verify_context {
  ...
  /// Current time, retrieved from the verifiers clock and cached for
   ...
   */
  basic_claim<json_traits> get_claim(
      bool in_header, std::error_code& ec) const {
    if (in_header) {
  ...
  /**
   * Helper method to get a claim of a specific type from the jwt in this
   * context
   * \param in_header check JWT header or payload sections
   ...
   */
  basic_claim<json_traits> get_claim(
      bool in_header, json::type t, std::error_code& ec) const {
...
struct equals_claim {
  ...
  void operator()(
      const verify_context<json_traits>& ctx, std::error_code& ec) const {
...
struct insensitive_string_claim {
  ...
  static std::string to_lower_unicode(
...
class verifier {
 public:
   ...
   *
   * This gets passed the current verifier, a reference to the decoded jwt, a
   * reference to the key of this claim, as well as a reference to an
   * error_code. The function checks if the actual value matches certain rules
   * (e.g. equality to value x) and sets the error_code if it does not. Once a
   * non zero error_code is encountered the verification stops and this
   * error_code becomes the result returned from verify
   */
  using verify_check_fn_t = std::function<void(
      const verify_ops::verify_context<json_traits>&, std::error_code& ec)>;
 ...
 public:
  ...
  explicit verifier(Clock c) : clock(c) {
    claims["exp"] = [](const verify_ops::verify_context<json_traits>& ctx,
                       std::error_code& ec) {
    ...
    claims["iat"] = [](const verify_ops::verify_context<json_traits>& ctx,
                       std::error_code& ec) {
    ...
    claims["nbf"] = [](const verify_ops::verify_context<json_traits>& ctx,
                       std::error_code& ec) {
   ...
   * Section 5.1](https://datatracker.ietf.org/doc/html/rfc7519#section-5.1),
   * This parameter is ignored by JWT implementations; any processing of this
   * parameter is performed by the JWT application. Check is case sensitive.
   *
   ...
   */
  verifier& with_type(
      const typename json_traits::string_type& type,
      std::locale locale = std::locale{}) {
    return with_claim(
        "typ",
        verify_ops::insensitive_string_claim<json_traits, true>{
   ...
   * This is helpful for implementating application specific authentication
   * checks such as the one seen in partial-claim-verifier.cpp
   *
   ...
   */
  verifier& with_claim(
      const typename json_traits::string_type& name, verify_check_fn_t fn) {
    claims[name] = fn;
    return *this;
  }
   ...
   */
  verifier& with_claim(
      const typename json_traits::string_type& name, basic_claim_t c) {
  ...
  void verify(const decoded_jwt<json_traits>& jwt, std::error_code& ec) const {
    ...
    verify_ops::verify_context<json_traits> ctx{
        clock.now(), jwt, default_leeway};
    for (auto& c : claims) {
...
class jwk {
 ...
 public:
   ...
   * Get key type claim
   *
   * This returns the general type (e.g. RSA or EC), not a specific algorithm
   * value.
   ...
   * in a valid token)
   */
  typename json_traits::string_type get_use() const {
    return get_jwk_claim("use").as_string();
  }
   ...
   * in a valid token)
   */
  typename json_traits::string_type get_curve() const {
    return get_jwk_claim("crv").as_string();
  }
   ...
   * \throw std::runtime_error If claim was not present
   * \throw std::bad_cast Claim was present but not a array (Should not happen
   * in a valid token)
   */
  typename json_traits::array_type get_x5c() const {
    return get_jwk_claim("x5c").as_array();
  };
   ...
   * in a valid token)
   */
  typename json_traits::string_type get_x5u() const {
    return get_jwk_claim("x5u").as_string();
  };
   ...
   * in a valid token)
   */
  typename json_traits::string_type get_x5t() const {
    return get_jwk_claim("x5t").as_string();
  };
   ...
   * \return Map of claims
   */
  typename json_traits::object_type get_claims() const {
...
class jwks {
 public:
   ...
   * \throw error::invalid_json_exception or underlying JSON implation error if
   * the JSON is invalid with regards to the JWKS specification
   */
  JWT_CLAIM_EXPLICIT jwks(const typename json_traits::string_type& str) {
    ...
    const details::map_of_claims<json_traits> jwks_json =
        json_traits::as_object(parsed_val);
 ...
 private:
  ...
  const_iterator find_by_kid(
      const typename json_traits::string_type& key_id) const noexcept {
...
/**
 * \brief Decode a token. This can be used to to help access important feild
 * like 'x5c' for verifying tokens. See associated example rsa-verify.cpp for
 * more details.
 *
 * \tparam json_traits JSON implementation traits
 * \tparam Decode is callable, taking a string_type and returns a string_type.
 *         It should ensure the padding of the input and then base64url decode
...
decoded_jwt<json_traits> decode(
    const typename json_traits::string_type& token, Decode decode) {
...
std::ostream& operator<<(
    std::ostream& os, const jwt::basic_claim<json_traits>& c) {
  return os << c.to_json();
}
