...
inline std::string toBase64(const std::string& input) {
  ...
  // sodium_base64_encoded_len(len, variant) 會回傳包含結尾 '\0'
  // 的最小緩衝區長度
  const size_t raw_len = input.size();
  const size_t b64_buf_len = sodium_base64_encoded_len(
 ...
 * @return std::vector<unsigned char> 解碼後的原始二進位資料 (長度 =
 * hexStr.size() / 2)
...
class AESManager {
 public:
  ...
  [[nodiscard]] std::string decrypt(const std::string& hexCipher) const {
    ...
    if (EVP_DecryptUpdate(ctx, plainBuf.data(), &len1, cipher, cipherLen) !=
            1 ||
